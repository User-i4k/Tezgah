...
export default function PatronSubeDashboard() {
    ...
    const analizleriYukle = async (subeId, isletmeId) => {
        ...
        const { data: sonFisler } = await supabase
            .from("sales")
            .select("id, toplam_tutar, created_at, personel_id, duzenlendi, duzenlendi_at")
            .eq("branch_id", subeId)
        ...
        const { data: gecmisData, error: gecmisHata } = await supabase
            .from("sales")
            .select("*, duzenlendi, duzenlendi_at")
            .eq("branch_id", subeId)
            .order("created_at", { ascending: false })
            .limit(100);
    ...
    useEffect(() => {
        const paneliHazirla = async () => {
            ...
            // INSERT: yeni sipariş → yenile
            const satisKanal = supabase
                .channel(`sube-satislar-${subeData.id}`)
                ...
                // UPDATE: personel düzenledi → sadece gecmisSiparisler'i güncelle
                .on("postgres_changes", { event: "UPDATE", schema: "public", table: "sales", filter: `branch_id=eq.${subeData.id}` },
                    (payload) => {
                        setGecmisSiparisler(prev => prev.map(s =>
                            s.id === payload.new.id
                                ? { ...s, toplam_tutar: payload.new.toplam_tutar, duzenlendi: payload.new.duzenlendi, duzenlendi_at: payload.new.duzenlendi_at }
                                : s
                        ));
                        setSonSatislar(prev => prev.map(s =>
                            s.id === payload.new.id
                                ? { ...s, toplam_tutar: payload.new.toplam_tutar, duzenlendi: payload.new.duzenlendi }
                                : s
                        ));
                        // Eğer bu sipariş detay modalında açıksa yeniden yükle
                        setSeciliSiparis(prev => {
                            if (prev?.id === payload.new.id) {
                                siparisDetayYukle(payload.new);
                ...
                // DELETE: personel sildi → listeden kaldır
                .on("postgres_changes", { event: "DELETE", schema: "public", table: "sales" },
                    (payload) => {
                        setGecmisSiparisler(prev => {
                            if (prev.some(s => s.id === payload.old.id)) {
                                setTimeout(() => analizleriYukle(subeData.id, isletmeData.id), 0);
                                return prev.filter(s => s.id !== payload.old.id);
                            }
                            return prev;
                        });
                        setSonSatislar(prev => prev.filter(s => s.id !== payload.old.id));
                        setSeciliSiparis(prev => prev?.id === payload.old.id ? null : prev);
                    })
                .subscribe();
    ...
    useEffect(() => {
        if (sube && isletme) analizleriYukle(sube.id, isletme.id);
    }, [enCokSatanlarAralik]);
    ...
    useEffect(() => {
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
            if (session?.user && session.user.id !== kullanici.id) {
                alert("🚨 GÜVENLİK UYARISI: Farklı bir patron hesabına giriş tespit edildi!");
    ...
    const urunEkleVeyaGuncelle = async (e) => {
        ...
        if (duzenlenenUrunId) {
            ...
            const { data, error } = await supabase.from("products").insert({
                business_id: isletme.id, branch_id: sube.id,
                isim: yeniUrun.isim.trim(), fiyat: parseFloat(yeniUrun.fiyat), kategori: yeniUrun.kategori
            }).select();
            if (error) alert("Hata: " + error.message);
    ...
    const kategoriSil = async (silinecekAd) => {
        ...
        if (!window.confirm(`"${silinecekAd}" kategorisini silmek istediğinize emin misiniz?`)) return;
        setKatIslemBekliyor(true);
            ...
            <header className="sticky top-0 z-40 bg-slate-950 border-b border-white/5 shadow-xl shadow-black/20">
                <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 h-14 sm:h-16">
                    <div className="flex items-center gap-3 sm:gap-4">
                        <button onClick={() => window.location.href = "/patron"}
                            className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-xl bg-white/10 hover:bg-white/20 text-white/60 hover:text-white transition-all text-sm">←</button>
                        <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <div>
                                <h1 className="text-white font-outfit font-black tracking-tighter leading-none text-base sm:text-lg">Tezgah<span className="text-indigo-500">.</span></h1>
                    ...
                    <nav className="hidden md:flex items-center gap-1 bg-white/5 rounded-xl p-1">
                        {sekmeListesi.map(s => (
                            <button key={s.id} onClick={() => setAktifSekme(s.id)}
                                className={`flex items-center gap-2 px-3 lg:px-4 py-2 rounded-lg text-xs font-outfit font-black uppercase tracking-widest transition-all ${aktifSekme === s.id ? "bg-white text-slate-900 shadow-sm" : "text-white/50 hover:text-white hover:bg-white/10"}`}>
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => analizleriYukle(sube.id, isletme.id)}
                ...
                <div className="fixed inset-0 z-50 flex md:hidden">
                    ...
                    <div className="relative w-72 max-w-[85vw] bg-slate-950 h-full flex flex-col p-6 shadow-2xl border-r border-white/5">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h1 className="text-white text-2xl font-outfit font-black tracking-tighter leading-none">Tezgah<span className="text-indigo-500">.</span></h1>
                        ...
                        <nav className="flex flex-col gap-1.5 flex-1">
                            {sekmeListesi.map(s => (
                                <button key={s.id} onClick={() => { setAktifSekme(s.id); setMenuAcik(false); }}
                                    className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all text-left group ${aktifSekme === s.id ? "bg-indigo-500/20 border border-indigo-500/30" : "hover:bg-white/5 border border-transparent"}`}>
                                    <span className="text-lg">{s.ikon}</span>
                                    <span className={`font-outfit font-black text-sm uppercase tracking-widest ${aktifSekme === s.id ? "text-indigo-300" : "text-white/60 group-hover:text-white"}`}>{s.isim}</span>
                                </button>
                            ))}
                        </nav>
                        <div className="pt-4 border-t border-white/5">
                            <button onClick={() => { analizleriYukle(sube.id, isletme.id); setMenuAcik(false); }}
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                                ...
                                <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-gray-100 flex items-center justify-between">
                                    ...
                                    <div className="flex bg-slate-100 p-1 rounded-xl gap-0.5">
                                        {[["bugun", "Gün"], ["hafta", "Hft"], ["ay", "Ay"]].map(([key, label]) => (
                                            <button key={key} onClick={() => setEnCokSatanlarAralik(key)}
                                                className={`text-[9px] sm:text-[10px] font-outfit font-black px-2.5 sm:px-3 py-1.5 rounded-lg transition-all uppercase tracking-widest ${enCokSatanlarAralik === key ? "bg-slate-900 text-white shadow-sm" : "text-slate-400 hover:text-slate-700"}`}>
                                                {label}
                                            </button>
                                        ))}
                                    </div>
                            ...
                            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                                <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-gray-100 flex items-center justify-between">
                                    ...
                                    <button onClick={() => setAktifSekme("gecmis")} className="text-[10px] font-outfit font-black text-indigo-600 hover:text-indigo-700 uppercase tracking-widest">Tümü →</button>
                                </div>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {s.duzenlendi && (
                                                            <span className="text-[8px] font-black text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-md uppercase">Düzenlendi</span>
                                                        )}
                                                    </div>
                                                    <p className="text-[10px] text-slate-400 font-medium mt-0.5">{tarihFormat(s.created_at)}</p>
                                                </div>
                                            </div>
                                            <button onClick={() => siparisDetayYukle(s)}
                        ...
                        <div className="xl:col-span-4">
                            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6 xl:sticky xl:top-24">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="font-outfit font-black text-slate-900 tracking-tight text-sm sm:text-base">{duzenlenenUrunId ? "Ürünü Düzenle" : "Yeni Ürün"}</h3>
                                    <button onClick={() => setKategoriModalAcik(true)}
                                ...
                                <form onSubmit={urunEkleVeyaGuncelle} className="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" required placeholder="Örn: Su Böreği" value={yeniUrun.isim} onChange={(e) => setYeniUrun({ ...yeniUrun, isim: e.target.value })}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <input type="number" required min="0" step="0.01" placeholder="0.00" value={yeniUrun.fiyat} onChange={(e) => setYeniUrun({ ...yeniUrun, fiyat: e.target.value })}
                                                className="w-full bg-slate-50 border-2 border-slate-200 focus:border-indigo-500 rounded-xl px-4 py-3 text-sm font-outfit font-black text-slate-900 outline-none transition-all placeholder:text-slate-300" />
                                        </div>
                                        <div className="relative">
                                            <label className="text-[9px] font-outfit font-black text-slate-500 uppercase tracking-[0.2em] block mb-1.5">Kategori</label>
                                            <button type="button" onClick={() => setKatMenuAcik(!katMenuAcik)}
                                                ...
                                                <div className="absolute top-full left-0 w-full mt-1.5 bg-white border-2 border-slate-200 rounded-xl shadow-xl p-1.5 z-[60]">
                                                    <div className="max-h-44 overflow-y-auto space-y-0.5">
                                                        {kategorilerListesi.map(k => (
                                                            <button key={k} type="button" onClick={() => { setYeniUrun({ ...yeniUrun, kategori: k }); setKatMenuAcik(false); }}
                                                                className={`w-full text-left px-3 py-2.5 rounded-lg text-xs font-outfit font-black uppercase tracking-widest transition-all ${yeniUrun.kategori === k ? "bg-indigo-500 text-white" : "text-slate-600 hover:bg-indigo-50 hover:text-indigo-700"}`}>
                                                                {k}
                                                            </button>
                                                        ))}
                                                    </div>
                                    ...
                                    <div className="pt-1">
                                        {duzenlenenUrunId ? (
                                            <div className="flex gap-2">
                                                <button type="button" onClick={() => { setDuzenlenenUrunId(null); setYeniUrun({ isim: "", fiyat: "", kategori: "Genel" }); }}
                                                    className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-600 font-outfit font-black py-3.5 rounded-xl text-[10px] uppercase tracking-widest transition-all">İptal</button>
                                                <button type="submit" disabled={urunEkleniyor}
                                                    className="flex-[2] bg-amber-500 hover:bg-amber-600 text-white font-outfit font-black py-3.5 rounded-xl text-[10px] uppercase tracking-widest shadow-lg shadow-amber-500/20 transition-all">Güncelle</button>
                                            </div>
                                        ) : (
                                            <button type="submit" disabled={urunEkleniyor}
                                            ...
                                            <div key={kat}>
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    {urunler.filter(u => (u.kategori || "Genel") === kat).map((urun) => (
                                                        <div key={urun.id} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${duzenlenenUrunId === urun.id ? "bg-indigo-50 border-indigo-200" : "bg-white border-slate-100 hover:border-slate-200 hover:shadow-sm"}`}>
                                                            <div className="flex-1 min-w-0">
                                                            ...
                                                            <div className="flex gap-1.5 ml-3">
                                                                <button onClick={() => { setDuzenlenenUrunId(urun.id); setYeniUrun({ isim: urun.isim, fiyat: urun.fiyat, kategori: urun.kategori || "Genel" }); }}
                                                                    className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center text-sm transition-colors">✏️</button>
                                                                <button onClick={() => urunSil(urun.id)}
                                                                    className="w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 flex items-center justify-center text-sm transition-colors">🗑️</button>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                        ...
                                        <div key={p.auth_id} className="bg-slate-50 rounded-2xl p-5 border border-slate-100 hover:border-indigo-100 hover:shadow-sm transition-all group">
                                            <div className="flex items-center gap-3 mb-4">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-[9px] font-outfit font-black text-slate-400 tracking-widest uppercase mt-0.5">{new Date(p.katilim_tarihi).toLocaleDateString("tr-TR")} katıldı</p>
                                                </div>
                                            </div>
                                            <div className="flex flex-col gap-2">
                                                ...
                                                {p.telefon && (
                                                    <a href={`https://wa.me/${p.telefon.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer"
                                    ...
                                    <div key={s.id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md hover:border-indigo-100 transition-all group">
                                        <div className="flex items-start justify-between mb-4">
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                {/* ── Düzenlendi etiketi ── */}
                                                {s.duzenlendi && (
                                                    <span className="text-[9px] font-black text-amber-600 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-lg uppercase tracking-wider flex items-center gap-1 w-fit">
                                                        ✏️ Düzenlendi
                                                        {s.duzenlendi_at && (
                                                            <span className="text-amber-400 font-medium normal-case tracking-normal">· {tarihFormat(s.duzenlendi_at)}</span>
                                                        )}
                                                    </span>
                                                )}
                                            </div>
                ...
                <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
                    <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden">
                        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-slate-50">
                            ...
                            <button onClick={() => setSeciliSiparis(null)} className="w-9 h-9 flex items-center justify-center bg-white hover:bg-slate-100 border border-slate-200 rounded-xl text-slate-500 text-sm">✕</button>
                        </div>

                        <div className="p-6 max-h-[70vh] overflow-y-auto">
                            {/* Düzenlendi bilgi bandı */}
                            {seciliSiparis.duzenlendi && (
                                <div className="mb-5 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-3">
                                    <span className="text-lg shrink-0">✏️</span>
                                    <div>
                                        <p className="text-xs font-black text-amber-700 uppercase tracking-widest">Bu sipariş düzenlendi</p>
                                        {seciliSiparis.duzenlendi_at && (
                                            <p className="text-[11px] text-amber-500 mt-0.5">
                                                {new Date(seciliSiparis.duzenlendi_at).toLocaleString("tr-TR")} tarihinde güncellendi
                                            </p>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="px-6 pb-6 pt-2">
                            <button onClick={() => setSeciliSiparis(null)}
                                className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-outfit font-black py-3.5 rounded-xl text-xs uppercase tracking-widest transition-colors">
                                Kapat
                            </button>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
                    <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl shadow-2xl border border-gray-100 flex flex-col max-h-[85vh]">
                        <div className="px-6 py-5 bg-slate-50 border-b border-gray-100 flex items-center justify-between shrink-0 rounded-t-3xl sm:rounded-t-2xl">
                            ...
                            <button onClick={() => { setKategoriModalAcik(false); setDuzenlenenKatAd(null); }} className="w-8 h-8 flex items-center justify-center bg-white hover:bg-slate-100 border border-slate-200 text-slate-500 rounded-xl text-sm">✕</button>
                        </div>
                        <div className="p-6 flex-1 overflow-y-auto relative">
                            ...
                            <div className="flex gap-2 mb-6">
                                <input type="text" placeholder="Yeni kategori adı..." value={yeniKatGirdisi} onChange={(e) => setYeniKatGirdisi(e.target.value)}
                                ...
                                <button onClick={kategoriEkle} className="bg-slate-900 hover:bg-slate-950 text-white px-4 py-2.5 rounded-xl text-[10px] font-outfit font-black uppercase tracking-widest">Ekle</button>
                            </div>
                            <div className="space-y-2">
                                {kategorilerListesi.map(k => (
                                    <div key={k} className="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50 hover:border-slate-200 transition-colors group">
                                        {duzenlenenKatAd === k ? (
                                            <input type="text" value={duzenlenenKatGirdisi} onChange={(e) => setDuzenlenenKatGirdisi(e.target.value)}
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            {duzenlenenKatAd === k ? (
                                                <button onClick={() => kategoriGuncelle(k)} className="w-7 h-7 rounded-lg text-green-600 flex items-center justify-center text-xs border border-green-200 hover:bg-green-50 bg-white">✓</button>
                                            ) : (
                                                <button onClick={() => { setDuzenlenenKatAd(k); setDuzenlenenKatGirdisi(k); }} className="w-7 h-7 rounded-lg text-slate-400 opacity-0 group-hover:opacity-100 flex items-center justify-center text-xs border border-slate-200 hover:bg-white">✏️</button>
                                            )}
                                            <button onClick={() => kategoriSil(k)} disabled={k === "Genel"}
